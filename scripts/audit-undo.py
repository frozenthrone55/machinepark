...
replacement = r'''function auditUndoButton(entry,change,changeIndex){
  ...
  return `<button class="btn small" type="button" data-undo-audit="1" data-audit-key="${esc(entry.auditKey||'')}" data-change-index="${changeIndex}" data-audit-label="${esc(change.entityLabel||change.entityType||'deze wijziging')}">Ongedaan maken</button>`
}
async function undoAuditChange(btn){
  ...
  const label=btn.dataset.auditLabel||'deze wijziging';
  if(!confirm(`Alleen de wijziging aan “${label}” ongedaan maken? Nieuwere wijzigingen aan hetzelfde veld worden nooit overschreven.`))return;
  btn.disabled=true;const oldText=btn.textContent;btn.textContent='Herstellen…';
  try{
    await adminFetch(AUDIT_LOG_URL,{method:'POST',body:JSON.stringify({auditKey:btn.dataset.auditKey,changeIndex:Number(btn.dataset.changeIndex)})});
    centralSync.etag=null;
    await centralPull({apply:true,quiet:false});
    await loadAuditLog();
    toast('Wijziging ongedaan gemaakt');
