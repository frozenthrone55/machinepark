...
if MARKER not in index:
    ...
    breakdown_summary_new = "function breakdownWorkSummary(b){if(b?.serviceVisitId){const sessions=Array.isArray(b?.serviceReportWorkSessions)?b.serviceReportWorkSessions:(Array.isArray(b?.workSessions)?b.workSessions:[]),sessionMinutes=sessions.reduce((sum,row)=>sum+Math.max(0,Math.round(Number(row?.minutes)||0)),0),minutes=Math.max(0,Math.round(Number(b?.serviceReportTotalMinutes)||sessionMinutes||Number(b?.hours||0)*60)),linked=[...(state.maintenance||[]),...(state.breakdowns||[])].filter(x=>(x?.serviceReportId||x?.serviceVisitId)===(b?.serviceReportId||b?.serviceVisitId)),unique=new Set(linked.map(x=>x?.deviceId).filter(Boolean)).size,count=Math.max(1,unique||Math.round(Number(b?.serviceReportDeviceCount||b?.serviceVisitDeviceCount||b?.batchSize)||1));if(minutes<=0)return '—';return `${minutes} min / ${count} ${count===1?'toestel':'toestellen'}`;}const hours=Number(b?.hours||0);if(!Number.isFinite(hours)||hours<=0)return '—';const liveCount=b?.batchId?state.breakdowns.filter(x=>x.batchId===b.batchId).length:0;const count=b?.batchId?Math.max(1,liveCount||Number(b.batchSize||0)):1;const minutes=Math.round(hours*60);return `${minutes} min / ${count} ${count===1?'toestel':'toestellen'}`}"
    ...
    maintenance_summary_new = "function maintenanceWorkSummary(m){if(m?.serviceVisitId){const sessions=Array.isArray(m?.serviceReportWorkSessions)?m.serviceReportWorkSessions:(Array.isArray(m?.workSessions)?m.workSessions:[]),sessionMinutes=sessions.reduce((sum,row)=>sum+Math.max(0,Math.round(Number(row?.minutes)||0)),0),minutes=Math.max(0,Math.round(Number(m?.serviceReportTotalMinutes)||sessionMinutes||Number(m?.hours||0)*60)),linked=[...(state.maintenance||[]),...(state.breakdowns||[])].filter(x=>(x?.serviceReportId||x?.serviceVisitId)===(m?.serviceReportId||m?.serviceVisitId)),unique=new Set(linked.map(x=>x?.deviceId).filter(Boolean)).size,count=Math.max(1,unique||Math.round(Number(m?.serviceReportDeviceCount||m?.serviceVisitDeviceCount||m?.batchSize)||1));if(minutes<=0)return '—';return `${minutes} min / ${count} ${count===1?'toestel':'toestellen'}`;}const hours=Number(m?.hours||0);if(!Number.isFinite(hours)||hours<=0)return '—';const count=m?.batchId?Math.max(1,state.maintenance.filter(x=>x.batchId===m.batchId).length):1;const minutes=Math.round(hours*60);return `${minutes} min / ${count} ${count===1?'toestel':'toestellen'}`}"
