...
(() => {
  ...
  window.machineparkServiceWorkSessionsEditor = (record,kind) => {const rows=sessionsFor(record),total=totalOf(rows);return `<div class="field full service-work-sessions" data-service-work-sessions><label>Werkdagen en tijd</label><input name="hours" type="hidden" value="${total}"><div class="service-work-session-list" data-service-work-session-list>${rows.map(rowHtml).join('')}</div><div class="service-work-session-actions"><button type="button" class="btn small" data-add-work-session>+ Dag toevoegen</button><strong data-service-work-total>Totaal: ${total} min</strong></div><div class="muted" style="font-size:11px">De totale tijd geldt voor de volledige ${kind==='breakdown'?'depannage':'onderhoud'}${record?.batchId?'groep':''}.</div></div>`;};
...
for needle in [MARKER,'Werkdagen en tijd','data-add-work-session','workSessionDate','workSessionMinutes','machineparkCollectWorkSessions','workSessions,hours:totalMinutes/60']:
