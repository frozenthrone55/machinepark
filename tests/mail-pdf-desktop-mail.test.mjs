...
test('Windows Mail PDF start Outlook Classic vanuit de oorspronkelijke gebruikersklik', () => {
  assert.match(builder,/machinepark-outlook-classic-bridge-v3/);
  assert.match(builder,/function machineparkIsWindowsDesktop\(\)/);
  assert.match(builder,/function machineparkOutlookPendingRequest\(context\)/);
  assert.match(builder,/function machineparkLaunchOutlookPending\(request\)/);
  assert.match(builder,/machinepark-outlook:\/\/compose\?file=/);
  assert.match(builder,/anchor\.click\(\)/);
  assert.match(builder,/const outlookRequest = machineparkIsWindowsDesktop\(\) \? machineparkOutlookPendingRequest\(context\) : null/);
  assert.match(builder,/machineparkLaunchOutlookPending\(outlookRequest\)/);
  assert.match(builder,/createDirectPdf\(context, outlookRequest\?\.fileName \|\| ''\)/);
  assert.match(builder,/Outlook openen…/);
});

test('Directe PDF kan vooraf gekozen Outlook bestandsnaam gebruiken', () => {
  assert.match(builder,/async function createDirectPdf\(context, forcedFilename = ''\)/);
  assert.match(builder,/forcedFilename \|\| `\$\{safeFilename/);
  assert.match(builder,/downloadFile\(file\)/);
});

test('Outlook Classic setup registreert lokaal protocol en wacht op de daarna gedownloade PDF', () => {
  ...
  assert.match(builder,/AddSeconds\(60\)/);
