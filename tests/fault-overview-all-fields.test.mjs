...
test('storingenoverzicht gebruikt alle actuele centrale detailwaarden correct en cacheveilig', () => {
  ...
  assert.equal(pkg.version, '1.68.11');
  ...
  const faultJsUrl = /\.\/fault-library\.js\?v=1\.68\.11-[a-f0-9]{12}/;
  const faultCssUrl = /\.\/fault-library\.css\?v=1\.68\.11-[a-f0-9]{12}/;
  ...
  assert.match(sw, /const CACHE='machinepark-v1\.68\.11-assets-[a-f0-9]{12}'/);
  assert.match(sw, /["']\.\/fault-library\.js\?v=1\.68\.11-[a-f0-9]{12}["']/);
  assert.match(sw, /["']\.\/fault-library\.css\?v=1\.68\.11-[a-f0-9]{12}["']/);
