(() => {
  ...
  function recordWorkMinutesForDate(item,date) {
    const specific=Math.max(0,Math.round(Number(item?.serviceItemMinutes)||0));
  ...
  async function collectItems() {
    ...
    for(const card of document.querySelectorAll('#serviceVisitDevices .service-visit-device')){
      ...
      for(const kind of ['maintenance','breakdowns','otherworks']){
        ...
        if(kind==='maintenance')out.push({...base,serviceItemMinutes:Math.max(0,Math.round(Number(panel?.querySelector('.sv-maintenance-hours')?.value||0)*60)),type:panel?.querySelector('.sv-maintenance-type')?.value||'Halfjaarlijks',notes:panel?.querySelector('.sv-maintenance-notes')?.value.trim()||''});
        else if(kind==='otherworks')out.push({...base,serviceItemMinutes:Math.max(0,Math.round(Number(panel?.querySelector('.sv-other-hours')?.value||0)*60)),serviceKind:'other',workTypeName:collectSvOtherWorkType(panel),priority:panel?.querySelector('.sv-other-priority')?.value||'Normaal',status:panel?.querySelector('.sv-other-status')?.value||'Open',issue:panel?.querySelector('.sv-other-issue')?.value.trim()||'',diagnosis:panel?.querySelector('.sv-other-diagnosis')?.value.trim()||'',solution:panel?.querySelector('.sv-other-solution')?.value.trim()||''});
        else out.push({...base,serviceItemMinutes:Math.max(0,Math.round(Number(panel?.querySelector('.sv-breakdown-hours')?.value||0)*60)),priority:panel?.querySelector('.sv-breakdown-priority')?.value||'Normaal',status:panel?.querySelector('.sv-breakdown-status')?.value||'Open',issue:panel?.querySelector('.sv-breakdown-issue')?.value.trim()||'',diagnosis:panel?.querySelector('.sv-breakdown-diagnosis')?.value.trim()||'',solution:panel?.querySelector('.sv-breakdown-solution')?.value.trim()||'',faultRef:typeof window.machineparkFaultRefFromCard==='function'?window.machineparkFaultRefFromCard(card):old?.faultRef||null});
  ...
  function finalRecord(header,item,{visit,serviceVisitId,visitNumberValue,visitRevision,reportId,reportNumberValue,reportRevision,now,batchSize}) {
    ...
    const itemMinutes=Math.max(0,Math.round(Number(item.serviceItemMinutes)||0));
    return {...record,date,time,technician,workSessions,hours:(itemMinutes>0?itemMinutes:totalMinutes)/60,batchId:serviceVisitId,batchSize,updatedAt:now,
