(() => {
  ...
  function reportWorkSessions(report) {
    ...
    const explicit=records.find(row=>Array.isArray(row.item?.serviceReportWorkSessions)&&row.item.serviceReportWorkSessions.length)?.item?.serviceReportWorkSessions;
    if(Array.isArray(explicit)&&explicit.length)return explicit.map(row=>({date:String(row.date||''),minutes:Math.max(0,Math.round(Number(row.minutes)||0))})).filter(row=>row.date&&row.minutes>0).sort((a,b)=>a.date.localeCompare(b.date));
    const byDate=new Map();
    for(const visit of report?.visits||[])for(const row of visitWorkSessions(visit)){if(!row.date||!Number(row.minutes))continue;byDate.set(row.date,(byDate.get(row.date)||0)+Number(row.minutes));}
    return [...byDate.entries()].map(([date,minutes])=>({date,minutes:Math.max(0,Math.round(minutes))})).sort((a,b)=>a.date.localeCompare(b.date));
  }
  ...
  function printRecordPageHtml(report,visit,row,index) {
    ...
    const sessions=reportWorkSessions(report),serviceMinutes=sessions.reduce((sum,session)=>sum+Number(session.minutes||0),0),deviceCount=Math.max(1,Number(report.deviceCount)||new Set((report.records||[]).map(entry=>entry.item?.deviceId).filter(Boolean)).size);
    ...
    return `<div class="service-visit-report service-report">
      <div class="service-report-print-summary">
        ...
        <div class="service-visit-report-meta">
          ...
          <div><small>Status</small><strong>${svEsc(typeof window.machineparkServiceReportStatus==='function'?window.machineparkServiceReportStatus(report):'Afgesloten')}</strong></div>
        </div>
  ...
  function partSection(kind,item={}) {
    ...
    return `<div class="service-visit-parts"><div class="service-visit-parts-head"><strong>Gebruikte onderdelen</strong><button type="button" class="btn small sv-add-part">+ Onderdeelregel</button></div><div class="muted" style="font-size:11px;margin:-3px 0 7px">Onderdelen blijven gekoppeld aan dit toestel en deze ${recordLabel}.</div><div class="usage-list sv-usage-list">${used}</div></div>
      ...
      <div class="field full sv-photo-editor" data-service-photo-recovery="1" data-service-photo-kind="${svEsc(kind)}" data-service-photo-record-id="${svEsc(item.id || '')}" data-existing-photos='${svEsc(JSON.stringify(item.photos || []))}'><label>Foto’s / video’s bij ${photoLabel}</label>${photoDraftHtml(item.photos || [])}<input class="sv-photo-files" type="file" accept="image/*,video/mp4,video/webm,video/quicktime" multiple><div class="muted" style="font-size:11px;margin-top:4px">Maximaal 10 foto’s en video’s samen per toestelregistratie.</div></div>`;
  }
  ...
  function collectUsed(panel){return [...(panel?.querySelectorAll('.sv-usage-list .usage-row')||[])].map(r=>({partId:r.querySelector('.usage-part')?.value||'',qty:normalizePartQuantity(r.querySelector('.usage-qty')?.value,1)})).filter(u=>u.partId&&u.qty>0);}
  function collectOneOff(panel){return [...(panel?.querySelectorAll('.service-visit-oneoff-row')||[])].map(r=>({supplier:String(r.querySelector('.sv-oneoff-supplier')?.value||'').trim(),supplierCode:String(r.querySelector('.sv-oneoff-code')?.value||'').trim(),description:String(r.querySelector('.sv-oneoff-description')?.value||'').trim(),qty:Math.max(0.001,normalizePartQuantity(r.querySelector('.sv-oneoff-qty')?.value,1))})).filter(p=>p.supplier||p.supplierCode||p.description);}
  ...
  async function collectPhotos(panel,kind,recordId,old=[],persistRecordId='') {
    ...
    const added=[];for(const file of files)added.push(await window.machineparkPrepareMediaFile(file,compressImage));
  ...
  function collectHeader() {
    ...
    return {...(existing||{}),id:activeVisitDraft.id,isDraft:true,draftRole:'header',draftKind:'serviceVisit',draftBatchId:activeVisitDraft.id,draftHeaderStore:activeVisitDraft.headerStore,locationKey:active?.key||'',locationLabel:active?.label||'',locations,activeLocationKey:active?.key||'',date:String(form?.elements.date?.value||''),time:String(form?.elements.time?.value||''),technician:String(form?.elements.technician?.value||'').trim(),workSessions:reportSessions,reportSessions,appendToVisitId:activeVisitDraft.appendToVisitId||'',appendToReportId:activeVisitDraft.appendToReportId||'',draftReportId:activeVisitDraft.draftReportId||'',linkedActionIds:Array.isArray(activeVisitDraft.linkedActionIds)?activeVisitDraft.linkedActionIds:[],editMode:Boolean(activeVisitDraft.editMode),createdAt:existing?.createdAt||activeVisitDraft.createdAt||now,updatedAt:now,draftSchema:4};
  }
  ...
  function stockUpdates(items,{editMode=false}={}) {
    const totals={};
    items.forEach(item=>{
      (item.usedParts||[]).forEach(u=>{const id=String(u?.partId||'').trim(),qty=normalizePartQuantity(u?.qty,0);if(id&&qty>0)totals[id]=normalizePartQuantity((totals[id]||0)+qty);});
      if(editMode&&(item.sourceRecordId||item.sourceUsedParts)){(item.sourceUsedParts||[]).forEach(u=>{const id=String(u?.partId||'').trim(),qty=normalizePartQuantity(u?.qty,0);if(id&&qty>0)totals[id]=normalizePartQuantity((totals[id]||0)-qty);});}
    });
    const now=new Date().toISOString(),updates=[];
    for(const[id,qty]of Object.entries(totals)){if(!qty)continue;const p=(state.parts||[]).find(x=>x.id===id);if(!p)throw new Error(`Onderdeel ${id} bestaat niet meer.`);updates.push({...p,stock:normalizePartQuantity(Number(p.stock||0)-qty),updatedAt:now});}
    return updates;
  }

  function finalRecord(header,item,{visit,serviceVisitId,visitNumberValue,visitRevision,reportId,reportNumberValue,reportRevision,now,batchSize,localDeviceCount,reportDeviceCount,reportSessions,reportTotalMinutes}) {
    const record={...item,id:item.sourceRecordId||item.id};['isDraft','draftRole','draftKind','draftBatchId','draftServiceKind','draftLocationKey','draftLocationLabel','draftSchema','targetVisitId','sourceRecordId','sourceUsedParts','serviceItemMinutes','serviceItemMinutesUnit'].forEach(k=>delete record[k]);
    ...
    const workSessions=(Array.isArray(reportSessions)?reportSessions:[]).map(row=>({date:String(row.date||''),minutes:Math.max(1,Math.round(Number(row.minutes)||0))})).filter(row=>row.date&&row.minutes>0);
    return {...record,date,time,technician,workSessions,hours:reportTotalMinutes/60,batchId:serviceVisitId,batchSize:localDeviceCount||batchSize,serviceVisitDeviceCount:localDeviceCount||batchSize,serviceVisitTotalMinutes:reportTotalMinutes,updatedAt:now,
      ...
      serviceReportWorkSessions:workSessions,serviceReportTotalMinutes:reportTotalMinutes,serviceReportDeviceCount:reportDeviceCount};
  }

  function finalizeDraftTransaction(header,allItems,selected,report) {
    const updates=stockUpdates(selected,{editMode:Boolean(header.editMode)}),now=new Date().toISOString(),reportId=report?.id||header.appendToReportId||header.draftReportId||uid('sr'),reportNumberValue=report?.number||reportNumber(reportId,header.date),reportRevision=report?Math.max(1,Number(report.revision)||1)+1:1;
    ...
    const finals=[],visitMeta=new Map();
    for(const [key,items] of groups){
      ...
      const localDeviceCount=Math.max(1,new Set([...(existing?.records||[]).map(row=>String(row.item?.deviceId||'')),...items.map(item=>String(item.deviceId||''))].filter(Boolean)).size);
      const built=items.map(item=>finalRecord(header,item,{visit:visitCtx,serviceVisitId,visitNumberValue,visitRevision,reportId,reportNumberValue,reportRevision,now,batchSize:items.length,localDeviceCount,reportDeviceCount,reportSessions,reportTotalMinutes}));
      finals.push(...built);visitMeta.set(serviceVisitId,{id:serviceVisitId,number:visitNumberValue,location:loc.label,locationKey:key,revision:visitRevision,deviceCount:localDeviceCount});
    }
    for(const item of retainedExisting){
      const serviceVisitId=item.serviceVisitId||uid('sv'),existingVisit=(report?.visits||[]).find(v=>v.id===serviceVisitId)||null;
      const localCount=Math.max(1,new Set((existingVisit?.records||[]).map(row=>row.item?.deviceId).filter(Boolean)).size||Number(item.serviceVisitDeviceCount)||1);
      finals.push({...item,date:header.date||item.date,time:header.time||item.time,technician:header.technician||item.technician,workSessions:reportSessions,hours:reportTotalMinutes/60,batchSize:localCount,serviceVisitDeviceCount:localCount,serviceVisitTotalMinutes:reportTotalMinutes,serviceReportWorkSessions:reportSessions,serviceReportTotalMinutes:reportTotalMinutes,serviceReportDeviceCount:reportDeviceCount,serviceReportDate:header.date||item.serviceReportDate,serviceReportTime:header.time||item.serviceReportTime,serviceReportTechnician:header.technician||item.serviceReportTechnician,serviceReportRevision:reportRevision,serviceReportClosedAt:now,updatedAt:now});
    ...
    const reportWorkState=typeof window.machineparkServiceWorkStateFromRecords==='function'?window.machineparkServiceWorkStateFromRecords(finals):'done';
  ...
  async function finalizeActiveVisit() {
    ...
    try{
      ...
      if(!(saved.header.reportSessions||saved.header.workSessions||[]).some(row=>row?.date&&Number(row?.minutes)>0))throw new Error('Voeg minstens één werkdag met servicetijd toe voor het volledige serviceverslag.');
  ...
  async function openServiceVisit(id='',draftId='',options={}) {
    ...
    activeVisitDraft={id:draftKey,headerStore:draftHeaderStore,header,items,report,locations,activeLocationKey:activeKey,reportSessions:Array.isArray(header?.reportSessions)?header.reportSessions:(Array.isArray(header?.workSessions)?header.workSessions:reportWorkSessions(report)),appendToReportId:report?.id||header?.appendToReportId||'',appendToVisitId:activeVisit?.id||header?.appendToVisitId||'',draftReportId:report?.id||header?.draftReportId||uid('sr'),linkedActionIds:[...new Set([...(Array.isArray(header?.linkedActionIds)?header.linkedActionIds:[]),...(typeof window.machineparkLinkedActionIdsForService==='function'?window.machineparkLinkedActionIdsForService(report?.id||header?.draftReportId||''):[])].map(String).filter(Boolean))],editMode,createdAt:header?.createdAt||new Date().toISOString(),persisted:Boolean(draftId),touched:false,finalizing:false,restoring:Boolean(header&&draftId)};
  ...
  async function deleteVisitDraft(id) {
    ...
    await new Promise((resolve,reject)=>{const tr=db.transaction(['maintenance','breakdowns'],'readwrite'),ms=tr.objectStore('maintenance'),bs=tr.objectStore('breakdowns');(header.draftHeaderStore==='maintenance'?ms:bs).delete(header.id);items.forEach(i=>(i.draftServiceKind==='maintenance'?ms:bs).delete(i.id));tr.oncomplete=()=>{scheduleCentralSync();resolve();};tr.onerror=()=>reject(tr.error);tr.onabort=()=>reject(tr.error);});if(header.draftReportId&&typeof window.machineparkClearServiceDraftActionLinks==='function')await window.machineparkClearServiceDraftActionLinks({reportId:header.draftReportId,date:header.date,locations:(Array.isArray(header.locations)?header.locations.map(x=>x&&x.label||x).filter(Boolean):[header.locationLabel].filter(Boolean))});await refreshVisitState();await syncVisitDraft();toast('Serviceconcept verwijderd');
  }

  function serviceReportLinkedActions(reportId) {
  ...
  function deleteServiceReportAtomic(report) {
    return new Promise((resolve,reject)=>{
      ...
      for(const [id,qty] of Object.entries(impact.totals)){
        ...
        updates.push({...part,stock:normalizePartQuantity(Number(part.stock||0)+Number(qty||0)),updatedAt:now});
      }
      const reportId=String(report.id),actionUpdates=serviceReportLinkedActions(reportId).map(action=>{
      ...
      let tr;
      try{tr=db.transaction(['maintenance','breakdowns','parts','actions'],'readwrite');}
      catch(error){reject(error);return;}
      const ms=tr.objectStore('maintenance'),bs=tr.objectStore('breakdowns'),ps=tr.objectStore('parts'),as=tr.objectStore('actions');
      updates.forEach(part=>ps.put(part));actionUpdates.forEach(action=>as.put(action));
      impact.rows.maintenance.forEach(record=>ms.delete(record.id));
      impact.rows.breakdowns.forEach(record=>bs.delete(record.id));
      tr.oncomplete=()=>{scheduleCentralSync();resolve({...impact,actionUnlinked:actionUpdates.length});};
  ...
  function serviceReportShouldUseIsolatedPrint() {
  ...
  function serviceReportPrintHeadStyles() {
  ...
  function showServiceReportDetails(id) {
    ...
    setTimeout(()=>{const form=document.getElementById('modalForm'),foot=form?.querySelector('.modal-foot'),cancel=document.getElementById('cancelModal'),submit=form?.querySelector('button[type="submit"]');if(!foot||!submit)return;if(cancel)cancel.style.display='none';submit.textContent='Sluiten';
      if(svCan('print')){const print=document.createElement('button');print.type='button';print.className='btn service-visit-print-btn';print.dataset.serviceReportId=report.id;print.textContent='🖨 Afdrukken';print.onclick=event=>{event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();void printServiceReport(report.id);};foot.insertBefore(print,submit);
  ...
  window.machineparkServiceVisitPdfModel = function(id) {
    ...
    const parts=mergedReportParts(report),sessions=reportWorkSessions(report),totalMinutes=sessions.reduce((sum,row)=>sum+row.minutes,0),fields=[
      {label:'Verslag',value:reportDisplayLabel(report)},{label:'Locaties',value:(report.visits||[]).map(v=>v.location||'—').join('\n')||'—'},{label:'Technieker',value:report.technician||'—'},{label:'Status',value:(typeof window.machineparkServiceReportStatus==='function'?window.machineparkServiceReportStatus(report):'Afgesloten')},
    ...
    for(const visit of report.visits||[]) {
      for(const row of visit.records||[]) {
        ...
        const serviceMinutes=totalMinutes,deviceCount=Math.max(1,Number(report.deviceCount)||new Set((report.records||[]).map(entry=>entry.item?.deviceId).filter(Boolean)).size);
        workPages.push({
          index:++workIndex,
          kind:row.kind,
    ...
    return {
      ...
      servicePrintLayout:{
        ...
        meta:[
          ...
          {label:'Status',value:(typeof window.machineparkServiceReportStatus==='function'?window.machineparkServiceReportStatus(report):'Afgesloten')},
        ],
