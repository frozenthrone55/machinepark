...
define('MP_MANUAL_MAX_BYTES', 40000000);
...
if($method==='PUT'){
    ...
    if($total<1||$total>12)manual_json(['error'=>'Ongeldig aantal PDF-blokken.'],400);
    $fileSize=(int)($_GET['fileSize']??0);
    if($fileSize<1||$fileSize>MP_MANUAL_MAX_BYTES)manual_json(['error'=>'De PDF is groter dan 40 MB of leeg.'],413);
    ...
    if($action==='finalize-upload'){
        ...
        for($i=0;$i<$total;$i++){
            ...
            fclose($in);
            if($written>MP_MANUAL_MAX_BYTES){fclose($out);@unlink($tmp);manual_remove_tree($dir);manual_json(['error'=>'De PDF is groter dan 40 MB.'],413);}
        }
        fclose($out);
