...
define('MP_MANUAL_MAX_BYTES', 20000000);
...
if($method==='PUT'){
    ...
    if($fileSize<1||$fileSize>MP_MANUAL_MAX_BYTES)manual_json(['error'=>'De PDF is groter dan 20 MB of leeg.'],413);
    ...
    if($action==='finalize-upload'){
        ...
        for($i=0;$i<$total;$i++){
            ...
            fclose($in);
            if($written>MP_MANUAL_MAX_BYTES){fclose($out);@unlink($tmp);manual_remove_tree($dir);manual_json(['error'=>'De PDF is groter dan 20 MB.'],413);}
        }
        fclose($out);
