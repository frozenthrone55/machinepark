...
function permissionSet(values = []) {
  return Object.fromEntries(ALL_PERMISSION_KEYS.map((key) => [key, values === 'all' || values.includes(key)]));
}
...
function diffList(beforeList = [], afterList = []) {
  ...
  for (const [id, item] of after) {
    if (!before.has(id)) added.push(item);
      ...
      const keys = changedRecordKeys(before.get(id), item).filter((key) => key !== 'updatedAt');
      if (keys.length) changed.push({ id, before: before.get(id), after: item, keys });
    }
  }
...
export function assertSnapshotWriteAllowed(before, after, roleValue, config = null, { owner = false } = {}) {
  ...
  const role = normalizeRole(roleValue, { owner, config });
  const permissions = permissionsForRole(role, config, { owner });
  if (!before) {
    if (owner || role === 'beheerder') return true;
    deny('Alleen de hoofdbeheerder of een beheerder kan de centrale database initialiseren.');
  }

  const devices = diffList(before.devices, after.devices);
  const maintenance = diffList(before.maintenance, after.maintenance);
  ...
  const serviceMutationAllowed =
    (maintenance.added.length && permissions['maintenance.add']) ||
    (maintenance.changed.length && permissions['maintenance.edit']) ||
    (maintenance.removed.length && permissions['maintenance.delete']) ||
    (breakdowns.added.length && permissions['breakdowns.add']) ||
    (breakdowns.changed.length && permissions['breakdowns.edit']) ||
    (breakdowns.removed.length && permissions['breakdowns.delete']);
