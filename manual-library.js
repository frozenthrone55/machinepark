(() => {
  const MANUAL_LIBRARY_URL = './synology/api/manual-library.php';
...
// machinepark-manual-native-sync-v1
  ...
  // machinepark-manual-device-brand-aliases-v1
  ...
  async function loadManualLibrary(force = false) {
    ...
    if (!force && manualLibraryLoaded && navigator.onLine === false) return manualLibrary;
  ...
  // machinepark-manual-library-sorting-v1
  ...
  function renderManualLibrary() {
    ...
    fillSelect(modelFilter, manualLibrary.filter((manual) => !selectedBrand || manualValueEquals(manual.brand, selectedBrand)).map((manual) => manual.model), 'Alle modellen');
    ...
    const visible = manualLibrary.filter((manual) => {
      ...
      if (selectedBrand && !manualValueEquals(manual.brand, selectedBrand)) return false;
      ...
      return true;
    }).sort((a, b) => manualSortDirection() * (manualCompareText(a.title, b.title) || manualCompareText(a.brand, b.brand) || manualCompareText(a.model, b.model) || manualCompareText(a.type, b.type) || manualCompareText(manualScopeText(a), manualScopeText(b)) || manualCompareText(a.id, b.id)));
  ...
  // machinepark-manual-chunk-upload-v1
  ...
  function openManualEditor(manual = null) {
    ...
    showModal(editing ? 'Handleiding bewerken' : 'Handleiding toevoegen', body, editing ? 'Wijzigingen opslaan' : 'Handleiding opslaan', async (formData) => {
      ...
      if (file) {
        if (typeof setCentralSyncStatus === 'function') setCentralSyncStatus('☁ PDF uploaden…', 'busy');
